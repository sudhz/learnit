...
const CourseDescription: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [descriptionResponse, courseResponse] = await Promise.all([
          axios.get(`http://localhost:5292/api/course/${id}/description`),
          axios.get(`http://localhost:5292/api/course/${id}`),
        ]);
